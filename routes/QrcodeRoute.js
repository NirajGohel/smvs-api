    ...
    try {
        ...
        if (listOfQrcode) {
            ...
            for (let q of listOfQrcode) {
                let presentUsers = await User.find({ _id: { $in: q.present } }, { firstName: 1, lastName: 1, middleName: 1, mobileNo: 1, callBy: 1, _id: 0 })
                let absentUsers = await User.find({ _id: { $nin: q.present } }, { firstName: 1, lastName: 1, middleName: 1, mobileNo: 1, callBy: 1, _id: 0 })
